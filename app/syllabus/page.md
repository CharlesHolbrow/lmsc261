---
title: "LMSC-261 | Syllabus"
---

# LMCS-261: Introduction to Computer Programming

<!--## Class Links
- [Canvas Link for LMSC-261-001](https://canvas.berklee.edu/courses/53378)
- [LMSC-261 Class Materials](https://github.com/LMSC-261/LMSC-261_Class_Materials)-->

## Class Time & Location

- Thursday, 5-7pm EST
- 150-122

## Instructor: Charles Holbrow

- Email: [cholbrow@berklee.edu](cholbrow@berklee.edu)
- Office Hours: TBD
- Pronouns: he/him/his
- [Course Chair](https://www.berklee.edu/people/michelle-darling)

<!-- ## Tutor Info TBD -->

## Description

In this course, students will learn the fundamentals of computer programming that will serve as a foundation for developing software applications and further exploring programming languages.

This targets computer coding fundamentals in the context of digital interactive multi-media

In this course you will

- Learn the basics of Python
- Potentially branch into other languages over the course of the semester
- Develop simple projects

## Preamble

The content and expectations of this course may change over the course of the semester.

## Expectations

All students in LMSC-261 must submit their problem sets and the final project using [GitHub](https://github.com/). This class focuses on computer programming and software application development. Students pursuing software engineering skills must learn to use a version control system such as [Git](https://git-scm.com/). Industry jobs commonly require version control skills.

You are also expected to:

- attend 12 lectures,
- take four quizzes,
- solve four problem sets,
- complete a mid-term project, and
- design and implement a final project.

## Learning Objectives

Upon completion of this course, students will be able to:

- Understand how computer programs are structured
- Use logic and problem-solving skills to design simple programs
- Comprehend the major concepts common to all programming languages, such as functions, loops, abstraction, algorithms, memory, data structures, and resource management
- Understand the software development cycle
- Use integrated development environments (IDEs) to build computer applications
- Use version control to organize programming projects
- Understand the basic Internet protocols such as TCP/IP and HTTP
- Write code to read and write data from web APIs
- Feel competent and confident in computational problem-solving

## Recommended Books

Most class materials will be online, and no books are required in this course, but the following books may be useful to students:

[**Automate the Boring Stuff with Python, Third Edition**](https://automatetheboringstuff.com/)  
Al Sweigart  
No Starch Press, 2025  
ISBN 1-7185-0340-7

[**Think Python: How to Think Like a Computer Scientist, Third Edition**](https://allendowney.github.io/ThinkPython/)  
Allen B. Downey  
Green Tea Press  
ISBN 1-491-93941-9

## Mid-term Project

In this course, students are required to work on a midterm project. The project is your opportunity to apply the knowledge you have gained in the course in a creative way. The project phase will start in week 6, and students will have two weeks to complete it.

| Week   | Milestone     
| :----- | :-------------
| Week 6 | Kickoff       
| Week 8 | Implementation

## Final Project

In this course, students are required to work on a final project. The project offers students the opportunity to test their newly acquired programming skills and develop their own software. The phase will start in week 9, and students will have four weeks to complete it.

| Week    | Milestone     
| :------ | :-------------
| Week 9  | Kick-Start    
| Week 10 | Proposal      
| Week 11 | Status Report 
| Week 12 | Implementation

## Quizzes

You must take four quizzes throughout the semester, covering lecture information, supplemental recordings, and possibly assigned readings. Students will have at least two weeks (14 days) to complete each quiz. Once the due date for a quiz passes, students will no longer be able to access or take it. Once you start the quiz, you will have one hour to complete it. You can take each quiz up to twice to maximize your grade.

## Problem Sets and Project Submission Policy

The instructor will post the problem set and project descriptions on the class material GitHub repository. The instructions will not be available on [Canvas](https://canvas.berklee.edu). <u>Please follow all project instructions.</u> If something is missing, you will be notified and allowed to resubmit, but with a late penalty.

### Problem Sets

- Students must submit their problem sets and projects using [GitHub](https://github.com) and not [Canvas](https://canvas.berklee.edu).
- Late problem sets will incur a penalty of 5 points each day.
- Problem sets and projects are due on the date specified before the class. If your class starts at 9:00 am, you must submit them before that class time.
- Your problem sets/projects are one day late if you submit them after the class starts on the date specified. For example, if your class begins at 9:00 am, and you submit your problem set at 9:01 am, your problem set is one day late, and you will lose 5 points.
- After the next class cycle (two days after the due date), your assignments/projects will be two days late, and you will lose 10 points.
- The instructor will not accept problem sets one week after the due date (cut-off day).
- After the cut-off day, you will receive a grade of 0.
- Makeup submissions after the due date will incur late penalties.
- No make-up submission is allowed for the problem sets after the cut-off time.
- The latest submission of your problem sets and projects will be graded.

### Mid-term and Final Projects

- The submission policy for mid-term and final projects is the same as that for problem sets.
- Late submission for the final project is not accepted, except in cases of emergency.

### General Advice

- No extra credit work exists in this course.
- Every late submission will burden you to complete the next ones.
- Students are encouraged to submit their assignments and projects on time, even if they are incomplete.
- If you have issues with your assignments, meet with the tutor or speak with your instructor immediately.
- Remember that the topics covered in this class are extensive and versatile.
- It is nearly impossible to master them all in a semester.
- Students should not expect to fully grasp all the concepts this class covers immediately; understanding will come through repeated exposure, practice, and perseverance.
- Do not get discouraged when you do not understand everything, and make it a habit to seek help when you are stuck.

## Grade Determination

The instructor will determine your final grade by considering the following components at the indicated weights:

| Description           | Grade |
| --------------------- | ----: |
| Biweekly Problem Sets |   25% |
| Biweekly Quizzes      |   25% |
| Mid-term Project      |   25% |
| Final Project         |   25% |

Problem sets and projects are evaluated using grading rubrics. The rubrics will typically assess the correctness, design, style, and documentation of your software programs. Keep in mind that [Canvas](https://canvas.berklee.edu) is used to give you feedback (click on the <u>Grades</u> on the left to view feedback) on your grades.

## Attendance Policy

Class attendance and participation are essential to your success in this course. Attendance is required for the entire semester, including final exam week. A student who misses the final exam, final project presentation, or final performance is subject to receiving a grade of F for the exam, project, or performance. You will need to plan accordingly for any end-of-semester travel plans. Students must be present on the day of the midterm and final project presentations. If students are absent, they will lose 5% of their total grade.

Your teacher will allow one (1) absence during the semester; after that, you will lose one grade increment for any additional absences. If your graded work for the semester qualifies you for a 'B' and you have three absences, your final grade will be a 'C+.'

Suppose you are going to miss a class. In that case, you need to notify your teacher in advance, or if that is not possible, as soon as possible after the class meeting you did not attend.

### Make-up work:

1. According to the Student Policy Handbook, students are responsible for submitting any missed class work or assignments resulting from absences.
2. The student is responsible for acquiring class content missed due to absences.
3. Professor's office hours are not intended to serve as a makeup class.
4. Extensions of due dates may be allowed at the professor’s discretion.
5. Submitting a late or missed assignment due to absence will be graded according to the course rubric.

### Add/Drop Period:

1. During the first two weeks of the Fall and Spring semesters, students may add or drop courses without penalty to their attendance records.
2. During the first week of the Summer semester, students may add or drop the course without penalty to their attendance record.
3. Students are expected to make up any assignments missed during add/drop periods.

### Late Entry/Early Leaving:

1. Students are considered late if they arrive in class after attendance has been taken.
2. A student will be marked absent if the departure is severely early (e.g., left the class right after the attendance check).
3. Students made an early departure if they did not tell the instructor before the class started and took off before it officially ended.
4. Three late or early departures count as one absence.

### Withdrawal Option:

1. If students anticipate exceeding the maximum allowable absences, they should consider withdrawing from the course during the designated withdrawal period to avoid a failing grade.

## AI Tools Policy

AI tools (ChatGPT, GitHub Copilot, etc.) are permitted when used transparently. Document AI assistance through code comments or assignment reflections: specify which tools were used, how outputs were verified, and what you learned.

Undisclosed AI usage is considered academic dishonesty. Some components may be designated "manual implementation required." Be prepared to explain and modify any submitted code.

When uncertain about the boundaries of AI use, students should consult with the instructor before submission.

## Class Topics

- Development Basics: Git, GitHub, Markdown
- Python: Variables & Operators, Control Flow, Functions, Lists
- Midterm: Project presentations
- Python: Libraries & Files
- Web Technologies
- Interactivity
- Agentic Coding
- Final: Project Presentations

<!--## Class Recordings
Recordings of class sessions will be on [YouTube](https://www.youtube.com/playlist?list=PLcX7R8HuFLWiP3YiPWSe-ZTzCXDocU6VG). Sign in to YouTube with your Berklee student account to view the lecture videos.-->

## Canvas vs. GitHub

- Students must submit all problem sets and projects to [GitHub](https://github.com) with their repository.
- All class materials will be posted on [GitHub](https://github.com) and not [Canvas](https://canvas.berklee.edu).
- The instructor will post grading and attendance on [Canvas](https://canvas.berklee.edu).
- Announcements will come from [Canvas](https://canvas.berklee.edu).

## Confidentiality

Screen captures or images from class, including photos of students or teachers, should not be used for social media without the permission of all individuals pictured.

## Berklee Policy

- [Grading Criteria/Rubric](https://college.berklee.edu/dept-grading-criteriarubric)
- [Berklee Equity Policy](https://docs.google.com/document/d/1lapSXuQyoxabZBmJKhV1xtQtgXBntzN3yGvUMrzOsKs/edit#bookmark=kix.8o2n4v7vppk5)
- [Honesty in Academic Work](https://docs.google.com/document/d/1lapSXuQyoxabZBmJKhV1xtQtgXBntzN3yGvUMrzOsKs/edit#heading=h.k7nvwvjsv5vc)

## Student Resources

- [Student Wellness Promotion and Services](https://www.berklee.edu/health/student-wellness)
- [Counseling Services](https://www.berklee.edu/counseling)
- [Accessibility Resources](https://www.berklee.edu/accessibility-resources)
- [Student Success Advising](https://college.berklee.edu/student-success-advising)
- [Career Advising](https://www.berklee.edu/career-center/career-advising)
- [Diversity, Equity, and Inclusion](https://www.berklee.edu/diversity)
- [Technology Service Desk](https://www.berklee.edu/service-desk)
- [Tutoring](https://www.berklee.edu/tutoring)
- [Stan Getz Library and Media Center](https://library.berklee.edu/)
- [Policy Handbook for Students](https://www.berklee.edu/policy-handbook-students)
